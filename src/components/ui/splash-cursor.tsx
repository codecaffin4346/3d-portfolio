  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (!gl) {
        ...
        return { gl: null, ext: null, isWebGL2 };
      }

      // Set up extensions and formats
      let halfFloat;
    ...
    // Material class (simplified version)
    class Material {
      ...
      setKeywords(keywords: string[]) {
        ...
        if (!program) {
          ...
          if (fragmentShader) {
            program = createProgram(this.vertexShader, fragmentShader) as WebGLProgram;
    ...
    // Program class (simplified version)
    class Program {
      uniforms: Record<string, WebGLUniformLocation> = {};
    ...
    // Create more shaders and programs
    ...
    // Set up WebGL buffer and drawing
    const blit = (() => {
      gl.bindBuffer(gl.ARRAY_BUFFER, gl.createBuffer() as WebGLBuffer);
      gl.bufferData(
        gl.ARRAY_BUFFER,
        new Float32Array([-1, -1, -1, 1, 1, 1, 1, -1]),
        gl.STATIC_DRAW
      );
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, gl.createBuffer() as WebGLBuffer);
      gl.bufferData(
    ...
    gl.clearColor(
      config.TRANSPARENT ? 0 : config.BACK_COLOR.r,
      config.TRANSPARENT ? 0 : config.BACK_COLOR.g,
      config.TRANSPARENT ? 0 : config.BACK_COLOR.b,
